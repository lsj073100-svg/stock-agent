...
with st.sidebar:
    ...
    # 고급 설정 (프롬프트 에디터)
    with st.expander("🛠️ 고급 설정 (프롬프트 에디터)", expanded=False):
        st.caption("AI의 성격, 분석 방향, 출력 형식을 직접 수정해 보세요.")
    ...
    if success and info:
        ...
        if run_button:
            if not api_key:
                ...
                with st.spinner("AI 에이전트 가동 중... (프롬프트 지침 준수 확인 중)"):
                    try:
                        ...
                        st.download_button(
                            label="📥 에퀴티 리서치 보고서(.md) 파일 다운로드",
                            data=response.text,
                            file_name=f"Equity_Research_Report_{ticker_input}_{date_str}.md",
                            mime="text/markdown",
                            use_container_width=True
                        )
                        st.success("보고서 분석 완료! 위 버튼을 눌러 마크다운 파일로 내보낼 수 있습니다.")
                    except Exception as e:
